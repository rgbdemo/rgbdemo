...
namespace ntk
{
  ...
  void load_ply_vertices(std::vector<cv::Point3f>& vertices, const char* ply_filename)
  {
    struct Vertex { float x, y, z; };
    const char *elem_names[] = { "vertex" };
    ply::PlyProperty vert_props[] = { /* list of property information for a vertex */
        {"x", Float32, Float32, offsetof(Vertex,x), 0, 0, 0, 0},
        {"y", Float32, Float32, offsetof(Vertex,y), 0, 0, 0, 0},
        {"z", Float32, Float32, offsetof(Vertex,z), 0, 0, 0, 0} };
    FILE* mesh_file = fopen(ply_filename, "r");
    ...
    for (int i = 0; i < ply_file->num_elem_types; i++)
    {
      ...
      if (ply::equal_strings("vertex", elem_name))
      {
        ...
        ply::setup_property_ply (ply_file, &vert_props[0]);
        ply::setup_property_ply (ply_file, &vert_props[1]);
        ply::setup_property_ply (ply_file, &vert_props[2]);

        /* grab all the vertex elements */
        for (int j = 0; j < elem_count; j++)
        {
          Vertex v;
          ply::get_element_ply (ply_file, &v);
          vertices[j] = cv::Point3f(v.x,v.y,v.z);
        }
      }
    }
  }

  void write_ply_vertices(const std::vector<cv::Point3f>& vertices, const char* ply_filename)
  {
  ...
  void write_ply_vertices(const std::vector<cv::Point3f>& vertices,
                          const std::vector<cv::Point3i>& colors,
                          const char* ply_filename)
  {
    ntk_assert(vertices.size() == colors.size(), "There must be one color for each vertex.");
    ...
    foreach_idx(i, vertices)
    {
      unsigned r = colors[i].x;
      unsigned g = colors[i].y;
      unsigned b = colors[i].z;
      ntk_assert(r < 256 && g < 256 && b < 256, "Bad color value, must be between 0 and 255.");
      ply_file << vertices[i].x << " " << vertices[i].y << " " << vertices[i].z
               << " " << r << " " << g << " " << b << "\n";
    }
    ply_file.close();
...
"invalid",
"int8", "int16", "PLY_Int32", "uint8", "uint16", "PLY_UInt32", "float32", "float64",
};
...
void binary_get_element(PlyFile *plyfile, char *elem_ptr)
{
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list == PLY_LIST) {          /* list */
      ...
      int len;
      char *str;
      ntk_ensure(fread (&len, sizeof(int), 1, fp), "Read error.");
      str = (char *) myalloc (len);
      ntk_ensure(fread (str, len, 1, fp), "Read error.");
      if (store_it) {
	char **str_ptr;
        item = elem_data + prop->offset;
	str_ptr = (char **) item;
...
)
{
  ...
  switch (type) {
    case Int8:
      ntk_ensure(fread (ptr, 1, 1, fp), "Read error.");
      *int_val = *((char *) ptr);
      *uint_val = *int_val;
      *double_val = *int_val;
      break;
    case Uint8:
      ...
      *double_val = *uint_val;
      break;
    case Int16:
      ntk_ensure(fread (ptr, 2, 1, fp), "Read error.");
      *int_val = *((short int *) ptr);
      *uint_val = *int_val;
      *double_val = *int_val;
      break;
    case Uint16:
      ...
      *double_val = *uint_val;
      break;
    case PLY_Int32:
      ntk_ensure(fread (ptr, 4, 1, fp), "Read error.");
      *int_val = *((int *) ptr);
      *uint_val = *int_val;
      *double_val = *int_val;
      break;
    ...
    case Float32:
      ...
      *uint_val = *double_val;
      break;
    case Float64:
      ntk_ensure(fread (ptr, 8, 1, fp), "Read error.");
      *double_val = *((double *) ptr);
      *int_val = *double_val;
      *uint_val = *double_val;
      break;
...
bool has_property(ply::PlyFile* ply_file, const char* element_name, const char* property_name)
{
  /* find an element in a plyfile's list */
  PlyElement *element = find_element(ply_file, element_name);

  if (!element)
    return false;

  int dummy = 0;
  PlyProperty * prop = find_property(element, property_name, &dummy);
