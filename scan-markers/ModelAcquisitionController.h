...
public:
  ...
  void setPoseEstimator(ntk::IncrementalPoseEstimatorFromMarkers* estimator) { m_pose_estimator = estimator; }
  ntk::RGBDModeler& modeler() { return m_modeler; }
  void setFloorPlane(const ntk::Plane& plane) { m_floor_plane = plane; }
  const ntk::RGBDImage& currentImage() { return m_current_image; }
  const ntk::Mesh& currentMesh() const { return m_current_mesh; }
...
protected:
  ...
  ntk::IncrementalPoseEstimatorFromMarkers* m_pose_estimator;
  QFuture<bool> m_new_frame_run;
  ntk::RGBDImage m_current_image;
  bool m_paused;
  ntk::Plane m_floor_plane;
  ntk::Mesh m_current_mesh;
};
