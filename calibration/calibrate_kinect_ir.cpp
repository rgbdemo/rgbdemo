                            ...
                            std::vector< DepthCalibrationPoint >& depth_values)
{
    ...
    for (int i_image = 0; i_image < global::images_list.size(); ++i_image)
    {
        ...
        ntk_ensure(image.data, "Could not load intensity image, did you grab the images in dual IR mode?");
    ...
    if (good_corners.size() < 1)
        fatal_error("Could not detect the checkerboard in any IR image.");
...
void calibrate_kinect_rgb(std::vector< std::vector<Point2f> >& stereo_corners)
{
    ...
    if (good_corners.size() < 1)
        fatal_error("Could not detect the checkerboard in any RGB image.");
...
{
    std::cout << "Writing into " << global::output_filename << " ..." << std::endl;
    FileStorage output_file (global::output_filename,
                             CV_STORAGE_WRITE);
...
void writeROSMatrix()
{
    ...
    calib.loadFromFile(global::output_filename.c_str());
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr << "No image found in " << global::images_dir.absolutePath().toStdString() << std::endl;
        exit (1);
    }

    if (std::string("calibration-#SERIAL.yml") == global::opt_output_file())
        global::output_filename = std::string("calibration-") + global::images_dir.dirName().toStdString() + ".yml";
    else
        global::output_filename = global::opt_output_file();
    ntk_dbg_print(global::output_filename, 1);
